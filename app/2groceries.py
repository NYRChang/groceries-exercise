# 2groceries.py
# This groceries app is based on the discussion in class
...
with open(csv_filepath, "r") as csv_file: # "r" means "open the file for reading"
    ...
    for row in reader:
        row["price"] = float(row["price"])  #change the price from string to row
        products.append(row) 
...
#Sorting by operator module - itemgetter
...
departments_list = []
for p in products:
    if p["department"] not in departments_list:
        departments_list.append(p["department"])
...
#Resaving the list as a sorted version of itself
departments_list = sorted(departments_list)

for department_name in departments_list:
    dept_products = [p for p in products if p["department"] == department_name]
    dept_products_count = len(dept_products)
    if dept_products_count > 1:
        label = "products"
    else:
        label = "product"
    print("+ " + department_name.title() + " (" + str(dept_products_count) + " " + label + ")")
